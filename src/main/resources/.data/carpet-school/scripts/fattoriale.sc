...
// GLOBALI
global_time = 300;
global_countdown = tick_time()-global_time;
global_domanda = 0;
...
__on_player_deals_damage(player, amount, entity) -> (
    if(tick_time()-global_countdown > global_time,
...
__on_player_takes_damage(player, amount, source, source_entity) -> (
    schedule(0,_(outer(amount),outer(player)) -> (
        if(player~'health' > 0 && tick_time()-global_countdown > global_time,
            global_quanto = amount;
            _domanda()
        );
    ))
